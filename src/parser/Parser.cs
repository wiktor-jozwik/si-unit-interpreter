...
public class Parser
{
    ...
    private bool TryParseFunctionStatement()
    {
        ...
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.LEFT_PARENTHESES}, _lexer.Token.Type,
        ...
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.RIGHT_PARENTHESES}, _lexer.Token.Type,
                _lexer.Token.Position);
        }

        if (!_CheckAndConsume(TokenType.RETURN_ARROW))
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.RETURN_ARROW}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
        ...
        if (statements == null)
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.LEFT_CURLY_BRACE}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
    ...
    private List<Parameter> TryParseParameters()
    {
        ...
        while (_CheckAndConsume(TokenType.COMMA))
        {
            ...
            {
                throw new ParserException(new HashSet<TokenType> {TokenType.IDENTIFIER}, _lexer.Token.Type,
                    _lexer.Token.Position);
            }
    ...
    private Parameter? TryParseParameter()
    {
        ...
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.COLON}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
    ...
    private Block? TryParseBlock()
    {
        ...
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.RIGHT_CURLY_BRACE}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
    ...
    private bool TryParseUnitDeclaration()
    {
        ...
        if (unitType == null)
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.LEFT_SQUARE_BRACKET}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
    ...
    private VariableDeclaration? TryParseVariableDeclaration()
    {
        ...
        {
            throw new ParserException(new HashSet<TokenType> {TokenType.ASSIGNMENT_OPERATOR}, _lexer.Token.Type,
                _lexer.Token.Position);
        }
    ...
    private IStatement? TryParseIfStatement()
    {
        ...
        while (_CheckAndConsume(TokenType.ELSE))
        {
            ...
            else
            {
                ...
                ifElseStatement = block ?? throw new ParserException(
                    new HashSet<TokenType> {TokenType.LEFT_CURLY_BRACE}, _lexer.Token.Type, _lexer.Token.Position);
            }
        }
    ...
    private IExpression? TryParseAdditiveExpression()
    {
        ...
        while (_additiveOperatorMap.TryGetValue(_lexer.Token.Type, out var additiveExpression))
        {
            ...
            leftMultiplicativeExpression =
                additiveExpression(leftMultiplicativeExpression, rightMultiplicativeExpression);
        }
